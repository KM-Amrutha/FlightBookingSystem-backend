...
@injectable()
export class FoodStatusChangeUseCase implements IFoodStatusChangeUseCase {
  constructor(
    @inject(TYPES_BOOKING_REPOSITORIES.FoodRepository)
    private _foodRepository: IFoodRepository
  ) {}

async execute(providerId: string, foodId: string): Promise<FoodResponseDTO> {
  if (!foodId) throw new validationError(FOOD_MESSAGES.FOOD_ID_REQUIRED);
